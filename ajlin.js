  ...
  addBtn.addEventListener('click', () => {
    ...
    popup.innerHTML = `
      ...
      <form id="character-form">
        ...
        <label>Generation:</label><br><input type="text" name="generation"><br>
    ...
    document.getElementById('cancel-btn').addEventListener('click', () => popupContainer.removeChild(popup));

    document.getElementById('character-form').addEventListener('submit', (e) => {
      e.preventDefault();
      const formData = new FormData(e.target);
      const characterData = {};
      formData.forEach((value, key) => characterData[key] = value);
      ...
      if (imgFile && imgFile.size > 0) imgURL = URL.createObjectURL(imgFile);
      card.innerHTML = `<img src="${imgURL}" alt="Character Image"><div class="character-name">${formData.get('name')}</div>`;
      ...
      card.addEventListener('click', () => {
        ...
        for (let key in characterData) {
          if (key !== 'image' && key !== 'name') detailsHTML += `<strong>${key.replace(/_/g,' ')}:</strong> ${characterData[key]}<br>`;
        }
        if (imgURL) detailsHTML += `<img src="${imgURL}" style="width:100%; margin-top:10px; border-radius:15px;">`;
        ...
        document.getElementById('close-detail').addEventListener('click', () => popupContainer.removeChild(detailPopup));
